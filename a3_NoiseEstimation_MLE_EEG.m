...
[y11,y12] = fcnGetData_EEG(T1,T2,ind);
...
while ~dn
    ...
    if ~mod(i,10000); 
        save BestZ3 z c l p y mn
    end
end
save BestZ3 z c l p y mn

%% find size of noise for o_ijt
load BestZ3 % z c l p y mn
p1 = []; p2 = [];
sig = linspace(1,7,500);
for i = 1:length(sig)
    [p1(i), p2(i)] = fcnGetOccNoise(sig(i),y11,y12,c,l,p,y); 
    disp([p1(i) p2(i)])
end
[ii,jj] = min(abs(p1-p2));
sig = sig(jj); 
save BestZ3 z c l p y mn sig
